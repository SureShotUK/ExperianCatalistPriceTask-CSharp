...
if (Email.DownloadEmailSpreadSheet()) // DownloadEmailSpreadsheet returns 'true' if an Email is found. 
{
    ...
    if (!Excel.MergeDataToSpreadsheet())
    {
        ...
        ErrorDetected = true;
        Email.SendEmail(ErrorDetected);
    }
    // No error detected, program has completed successfully and an Email will be sent. 
    else { ErrorDetected = false; Email.SendEmail(ErrorDetected); }
}
...
else
{
    // Send email.
    ErrorDetected = true;
    Email.SendEmail(ErrorDetected); // The SendEmail function uses a boolean to determine if an error was detected. 
}